		...
		if( 1  || has("host-webworker")){
			...
			req.getText = function(url, async, onLoad){
				...
				if(xhr.status == 200 || (!location.host && !xhr.status)){
					...
					// XWT added loader error message.
					console.error("XWT - DOJO LOADER: Unable to load: ", url, " Error: ", xhr.status);
					...
					if(module.isXd){
						...
						if( 1 ){
							try{
								...
								xhrCallback(""); // XWT:  ADDED BY RCG FOR JARED
							}
						}else{
							req.getText(url, legacyMode!=sync, xhrCallback);
						}
...
define(["../has", "./config", "require", "module"], function(has, config, require, module){
	...
	dojo.version = {
		...
		major: 1, minor: 10, patch: 4, flag: "4-XWT",
			...
			if(lang.isString(method)){
				...
				return function() {
					try{
						...
						console.error('Error executing: ', scope[method].toString(), e);
					}
				}; // Function
			}
		...
		onEvent = function(){
			...
			while(isDomReady && (!domReady || domReady._Q.length == 0) && (require.idle ? require.idle() : true) && loadQ.length){
				...
				try{
					...
					// XWT - Don't throw the error
					console.warn("XWT - Dojo Reported error in dojo.ready/dojo.addOnLoad: ", e.toString(), e, f.toString());
					//throw e;
				}
			}
