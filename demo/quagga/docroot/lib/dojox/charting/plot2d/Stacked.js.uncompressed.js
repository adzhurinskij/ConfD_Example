	...
	return declare("dojox.charting.plot2d.Stacked", Default, {
		...
		buildSegments: function(i, indexed){
			...
			for(var j = min; j <= max; j++){
				var value = indexed ? commonStacked.getIndexValue(this.series, i, j, isNullValue) :
						commonStacked.getValue(this.series, i, run.data[j] ? run.data[j].x : null, isNullValue);
