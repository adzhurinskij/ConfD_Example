                  ...
                  confd_hkeypath_t *keypath)
{
    ...
    OK(maapi_create(maapi_socket, tctx->thandle,
                    "%s/user{%x}", AAA_USERS, user));

    snprintf(buf, sizeof(buf)-5, "/var/confd/home/%s", CONFD_GET_BUFPTR(user));
    OK(maapi_set_elem2(maapi_socket, tctx->thandle, buf,
                       "%s/user{%x}/homedir", AAA_USERS, &(keypath->v[0][0])));
    strcat(buf, "/.ssh");
    OK(maapi_set_elem2(maapi_socket, tctx->thandle, buf,
