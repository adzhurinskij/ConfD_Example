...
define([
  ...
  funs.getValue = function(args /*{th, path}*/) {
    ...
    return jsonRpc.call('get_value', {
      ...
      log(args.path + ' is now set to: ' + result.value + '.');
    });
  };

  funs.getValues = function(args /*{th, path}*/) {
    log('Get values for ' + args.path + ' in ' + args.th + ' transaction...');
    if (typeof args.th === 'string') {
      args.th = ths[args.th];
    }
    return jsonRpc.call('get_values', {
      th: args.th,
      path: args.path,
      leafs: leafs
    }).done(function(result) {
  ...
  funs.subscribeCdboper = function(args /*{path, handle}*/) {
    log('Subcribing to cdboper changes of ' + args.path +
        ' (with handle ' + args.handle + ')...');
    return jsonRpc.call('subscribe_cdboper', {
      comet_id: comet2.id,
      path: args.path,
      handle: args.handle,
    }).done(function(result) {
  ...
  Promise.resolve([
    ...
    function() {
      var handle = comet2.id + '-addr_in_use';
      comet2.on(handle, function(msg) {
        log('>>> Cdboper Notification >>>\n' +
            JSON.stringify(msg, null, 2) +
            '\n<<< Cdboper Notification <<<');
      });
      return funs.subscribeCdboper({th: 'read', path: pathx, handle: handle});
    },
    funs.newWebuiTrans,
    function() {
